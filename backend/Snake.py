...
try:
    from backend.models.hybrid import CGPGenome as HybridCGPGenome, HybridSnakeAI, CGPEvolution as HybridCGPEvolution
    from models.CGP import SnakeGame as CGPSnakeGame, SnakeAI as CGPSnakeAI, CGPEvolution as CGPEvolution, CGPGenome
    from models.CGP import load_genome
...
class SnakeGame:
    def __init__(self, algorithm: str, ai_controller=None):
    ...
    def get_action(self):
        """Get the next action based on the selected algorithm"""
        if self.game_over:
            return None
        ...
        try:
            if self.algorithm == 'a_star':
                path = self.pathfinder.a_star(self.snake[0], self.food, set(self.snake[1:]))
                if path:
                    next_pos = path[0]
                    dx = next_pos[0] - self.snake[0][0]
                    dy = next_pos[1] - self.snake[0][1]
                    return self._action_to_direction((dx, dy))
                return self._rule_based_action()
            
            elif self.algorithm == 'greedy':
                path = self.pathfinder.greedy_best_first(self.snake[0], self.food, set(self.snake[1:]))
                if path:
                    next_pos = path[0]
                    dx = next_pos[0] - self.snake[0][0]
                    dy = next_pos[1] - self.snake[0][1]
                ...
                path = self.pathfinder.dijkstra(self.snake[0], self.food, set(self.snake[1:]))
                if path:
                    next_pos = path[0]
                    dx = next_pos[0] - self.snake[0][0]
                    dy = next_pos[1] - self.snake[0][1]
            ...
            elif self.algorithm in ['hybrid', 'cgp'] and self.ai_controller:
                return self.ai_controller.get_action(self._get_simple_state())
            ...
            self.peak_memory = max(self.peak_memory, peak / 10**6)  # Convert to MB
            tracemalloc.stop()
    ...
    async def step(self):
        ...
        head = self.snake[0]
        new_head = (
            (head[0] + self.direction.value[0]) % self.width,
            (head[1] + self.direction.value[1]) % self.height
        )
    ...
    def _action_to_direction(self, action):
        """Convert an action index, tuple, or Direction to a Direction enum"""
        directions = [Direction.UP, Direction.DOWN, Direction.LEFT, Direction.RIGHT]
        if isinstance(action, Direction):
            return action
        elif isinstance(action, tuple):
            for direction in directions:
                if direction.value == action:
                    return direction
    ...
    def get_metrics(self):
        ...
        return {
            ...
            "decision_time": np.mean(self.decision_times) if self.decision_times else 0,
            "path_efficiency": np.mean(self.path_lengths) / np.mean(self.optimal_path_lengths) if self.optimal_path_lengths else 0,
...
async def run_simulation(algorithm: str, hybrid_genome: Optional[HybridCGPGenome] = None, cgp_genome: Optional[CGPGenome] = None):
    ...
    game = SnakeGame(algorithm=algorithm, ai_controller=HybridSnakeAI(hybrid_genome, GRID_WIDTH, GRID_HEIGHT) if algorithm == "hybrid" and hybrid_genome else None)
